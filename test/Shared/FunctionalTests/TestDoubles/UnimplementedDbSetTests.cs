...
namespace System.Data.Entity.TestDoubles
{
    ...
    public class UnimplementedDbSetTests : FunctionalTestBase
    {
        public class UnimplementedDbSet<T> : DbSet<T>
            where T : class
        {
            public new IQueryable<T> Include<TProperty>(Expression<Func<T, TProperty>> path)
            {
                // MUST me added due to IncludeDbQuery`2 features
                return QueryableExtensions.Include(this, path);
            }
        }
