...
<configuration>
  <configSections>
    <section name='entityFramework' type='System.Data.Entity.Internal.ConfigFile.EntityFrameworkSection, Z.EntityFramework.Classic' />
  </configSections>
  <entityFramework>
    <providers>
...
";
        ...
        public BasicDesignTimeScenarios()
        {
            ...
            var pathToEFAssembly = Path.Combine(_outputDirectory, "Z.EntityFramework.Classic.dll");
            File.Copy(new Uri(typeof(DbContext).Assembly().CodeBase).LocalPath, pathToEFAssembly);
            var pathToEFSqlAssembly = Path.Combine(_outputDirectory, "Z.EntityFramework.Classic.SqlServer.dll");
            ...
            {
#if NETSTANDARD
                var pathToNetStandardAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(x => x.GetName().Name == "netstandard").Location;
#endif
                ...
                var results = compiler.CompileAssemblyFromSource(
                    new CompilerParameters(
                        new[]
                        {
                            pathToEFAssembly, pathToEFSqlAssembly
#if NETSTANDARD
                            , pathToNetStandardAssembly
#endif
                        },
