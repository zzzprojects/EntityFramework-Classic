...
{
    /// <summary>
    ///     Specifies the related objects to include in the query results and stay in the Include chain to the
    ///     TPropertyCurrent.
    /// </summary>
    ...
    public static IncludeDbQuery<TQuery, TPropertyCurrent> AlsoInclude<TQuery, TPropertyCurrent, TProperty>(this IIncludeDbQuery<TQuery, TPropertyCurrent> @this, Expression<Func<TPropertyCurrent, TProperty>> path)
    {
        string include;
        ...
        return new IncludeDbQuery<TQuery, TPropertyCurrent>(@this.Include(include), @this.IncludePath);
    }
    ...
    /// <param name="path">A lambda expression representing the path to include.</param>
    /// <returns>A new IncludeDbQuery&lt;TQuery, TProperty&gt; with the defined query path.</returns>
    public static IncludeDbQuery<TQuery, TPropertyCurrent> AlsoInclude<TQuery, TPropertyCurrent, TProperty>(this IIncludeDbQuery<TQuery, IEnumerable<TPropertyCurrent>> @this, Expression<Func<TPropertyCurrent, TProperty>> path)
    {
        string include;
        if (!DbHelpers.TryParsePath(path.Body, out include) || include == null)
        {
