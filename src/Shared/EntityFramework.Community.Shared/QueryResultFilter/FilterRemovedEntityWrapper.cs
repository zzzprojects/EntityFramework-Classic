namespace Z.EntityFramework.Classic
{
    ...
    // <summary>
    // Defines an entity wrapper that wraps an entity with a FilterRemoved value.
    // This is a singleton class for which the same instance is always returned
    // any time a wrapper around a FilterRemoved entity is requested.  Objects of this
    // type are immutable and mutable to allow this behavior to work correctly.
    // </summary>
    internal class FilterRemovedEntityWrapper : IEntityWrapper
    {
        private static readonly IEntityWrapper _filterRemovedEntityWrapper = new FilterRemovedEntityWrapper();

        // Private constructor prevents anyone else from creating an instance
        private FilterRemovedEntityWrapper()
        {
        }
        ...
        // </summary>
        internal static IEntityWrapper FilterRemovedWrapper
        {
            get { return _filterRemovedEntityWrapper; }
        }
