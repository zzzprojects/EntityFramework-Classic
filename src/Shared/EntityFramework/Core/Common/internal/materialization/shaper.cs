...
namespace System.Data.Entity.Core.Common.Internal.Materialization
{
    ...
    internal abstract class Shaper
    {
        ...
        public IEntityWrapper HandleEntityNoTracking<TEntity>(IEntityWrapper wrappedEntity)
        {
            ...
            if (Context.ContextOptions._queryResultFilterConfiguration.IsValueCreated
                && Context.ContextOptions._queryResultFilterConfiguration.Value.IsFilterRemoved(wrappedEntity))
            {
                wrappedEntity = FilterRemovedEntityWrapper.FilterRemovedWrapper;
            }

            if (!(wrappedEntity is FilterRemovedEntityWrapper))
            {
            ...
            Func<Shaper, IEntityWrapper> constructEntityDelegate, EntityKey entityKey, EntitySet entitySet)
        {
            ...
            else
            {
                    ...
                    && !existingEntry.IsKeyEntry)
                {
                    ...
                    // Z.EntityFramework.Classic: QueryResultFilter
                    if (Context.ContextOptions._queryResultFilterConfiguration.IsValueCreated
                        && Context.ContextOptions._queryResultFilterConfiguration.Value.IsFilterRemoved(result))
                    {
                        result = FilterRemovedEntityWrapper.FilterRemovedWrapper;
                    }
                }
                else
                {
                    ...
                    if (!(result is FilterRemovedEntityWrapper))
                    {
                        RegisterMaterializedEntityForEvent(result);
                        if (null == existingEntry)
                        {
