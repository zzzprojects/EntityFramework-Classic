            ...
            IEntityWrapper wrappedSource, AssociationEndMember targetMember, IList targets, bool setIsLoaded)
        {
            ...
            try
            {
                if (targets != null)
                {
                    ...
                    ILookup<EntityKey, RelationshipEntry> sourceKeyRelationships = null;
                    ...
                    foreach (var someTarget in targets)
                    {
                        ...
                        if (wrappedTarget is FilterRemovedEntityWrapper)
                        {
                            continue;
                        }
                        ...
                        else
                        {
                            ...
                            if (sourceKeyRelationships == null)
                            {
                                sourceKeyRelationships = GetRelationshipLookup(context.ObjectStateManager, associationSet, sourceMember, sourceKey);
                            }

                            if (
                                !TryUpdateExistingRelationships(
                                    context, mergeOption, associationSet, sourceMember, sourceKeyRelationships, wrappedSource, targetMember, targetKey,
