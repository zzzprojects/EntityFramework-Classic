    ...
    public class SqlServerMigrationSqlGenerator : MigrationSqlGenerator
    {
...
#if NETSTANDARD
            IEnumerableExtensions.Each<object>(DetectHistoryRebuild(migrationOperations), (o => GenerateByType(o)));
#else
        ...
        private void Generate(ParameterModel parameterModel, IndentedTextWriter writer)
        {
            ...
            if (parameterModel.DefaultValue != null)
            {
                writer.Write(" = ");
#if NETSTANDARD
                writer.Write(GenerateByType(parameterModel.DefaultValue));
#else
        ...
        protected virtual void Generate(AddColumnOperation addColumnOperation)
        {
            ...
            using (var writer = Writer())
            {
                    ...
                    && !column.StoreType.EqualsIgnoreCase("timestamp"))
                {
                    ...
                    {
#if NETSTANDARD
                        writer.Write(GenerateByType(column.ClrDefaultValue));
#else
        ...
        protected virtual void Generate(AlterColumnOperation alterColumnOperation)
        {
            ...
            using (var writer = Writer())
            {
                    ...
                    || !string.IsNullOrWhiteSpace(column.DefaultValueSql))
                {
                    ...
                    writer.Write(
                                            (column.DefaultValue != null)
                                                ? GenerateByType(column.DefaultValue)
        ...
        protected internal virtual void Generate(ColumnModel column, IndentedTextWriter writer)
        {
            ...
            if (column.DefaultValue != null)
            {
                writer.Write(" DEFAULT ");
#if NETSTANDARD
                writer.Write(GenerateByType(column.DefaultValue));
#else
...
#if NETSTANDARD
        private string GenerateByType(object defaultValue)
        {
            if (defaultValue is byte[] a) return Generate(a);
            else if (defaultValue is bool b) return Generate(b);
            else if (defaultValue is DateTime c) return Generate(c);
            else if (defaultValue is DateTimeOffset d) return Generate(d);
            else if (defaultValue is Guid e) return Generate(e);
            else if (defaultValue is string f) return Generate(f);
            else if (defaultValue is TimeSpan g) return Generate(g);
            else if (defaultValue is DbGeography h) return Generate(h);
            else if (defaultValue is DbGeometry i) return Generate(i);
            else if (defaultValue is UpdateDatabaseOperation j) Generate(j);
            else if (defaultValue is CreateProcedureOperation l) Generate(l);
            else if (defaultValue is AlterProcedureOperation m) Generate(m);
            else if (defaultValue is DropProcedureOperation o) Generate(o);
            else if (defaultValue is CreateTableOperation p) Generate(p);
            else if (defaultValue is AlterTableOperation q) Generate(q);
            else if (defaultValue is AddForeignKeyOperation r) Generate(r);
            else if (defaultValue is DropForeignKeyOperation s) Generate(s);
            else if (defaultValue is CreateIndexOperation t) Generate(t);
            else if (defaultValue is DropIndexOperation u) Generate(u);
            else if (defaultValue is AddPrimaryKeyOperation v) Generate(v);
            else if (defaultValue is DropPrimaryKeyOperation x) Generate(x);
            else if (defaultValue is AddColumnOperation y) Generate(y);
            else if (defaultValue is DropColumnOperation z) Generate(z);
            else if (defaultValue is AlterColumnOperation x1) Generate(x1);
            else if (defaultValue is DropTableOperation x2) Generate(x2);
            else if (defaultValue is SqlOperation x3) Generate(x3);
            else if (defaultValue is RenameColumnOperation x4) Generate(x4);
            else if (defaultValue is RenameIndexOperation x5) Generate(x5);
            else if (defaultValue is RenameTableOperation x6) Generate(x6);
            else if (defaultValue is RenameProcedureOperation x7) Generate(x7);
            else if (defaultValue is MoveProcedureOperation x8) Generate(x8);
            else if (defaultValue is MoveTableOperation x9) Generate(x9);
            else if (defaultValue is HistoryOperation x10) Generate(x10);
            else if (defaultValue is HistoryRebuildOperationSequence x11) Generate(x11);

            // Base types
            else if (defaultValue is ProcedureOperation n) Generate(n);
            else if (defaultValue is MigrationOperation k) Generate(k);
